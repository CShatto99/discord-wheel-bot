...
const spin: Command = {
	...
	async execute(interaction: Interaction) {
		...
		for (let i = 0; i < spinCount && items.length > 0; i++) {
			...
			let remainingItemsMsg: string;
			if (items.length > MAX_ITEM_DISPLAY) {
				remainingItemsMsg = `${items
					.slice(0, MAX_ITEM_DISPLAY)
					.map((i, idx) => `**${idx + 1}.** ${i}`)
					.join("\n")}\n…and ${items.length - MAX_ITEM_DISPLAY} more`;
			} else if (items.length > 0) {
				remainingItemsMsg = items
					.map((i, idx) => `**${idx + 1}.** ${i}`)
					.join("\n");
			} else {
				remainingItemsMsg = "✨ None left!";
			}
